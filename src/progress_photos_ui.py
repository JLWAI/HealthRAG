    ...
    with st.form("photo_upload_form", clear_on_submit=True):
        ...
        with col2:
            weight_lbs = st.number_input(
                ...
                step=0.1,
                help="Your weight on this date (optional)"
            )
            ...
            phase = st.selectbox(
                ...
                help="Your diet phase when this photo was taken"
            )

        notes = st.text_area(
            "Notes (optional)",
            placeholder="How are you feeling? Any observations?",
            help="Add notes about this photo"
        )

        uploaded_file = st.file_uploader(
            "Choose a photo",
            type=["jpg", "jpeg", "png"],
            help="Upload a progress photo (JPG or PNG)"
        )
...
def render_photo_gallery():
    ...
    if not photos:
        st.info("📷 No progress photos yet. Upload your first photo above!")
        return
        ...
        for j, col in enumerate(cols):
            if i + j < len(photos):
                ...
                with col:
                    ...
                    if photo.notes:
                        st.caption(f"📝 {photo.notes[:50]}{'...' if len(photo.notes) > 50 else ''}")

                    # Delete button
                    if st.button(f"🗑️ Delete", key=f"delete_{photo.photo_id}", use_container_width=True):
                        if tracker.delete_photo(photo.photo_id):
                            st.success(f"✅ Photo deleted")
                            st.rerun()
                        else:
...
def render_photo_comparison():
    ...
    if len(photos) < 2:
        st.info("📷 Upload at least 2 photos to compare progress")
        return
    ...
    if st.button("🔍 Compare Photos", type="primary", use_container_width=True):
        ...
        if not comparison:
            st.error("❌ Could not load photos for comparison")
            return
...
def render_latest_photos():
    ...
    if not latest_photos:
        st.info("📷 No progress photos yet. Upload your first photo to track visual progress!")
        return
