...
**⚠️ DEPLOYMENT:** Pushes to `main` don't auto-deploy. Deploy to homelab manually with `./deploy-to-homelab.sh`. Still batch changes and run `./test-checklist.sh` before pushing — deploy once per feature, not per fix. See `docs/TESTING.md` for local-first workflow details.
